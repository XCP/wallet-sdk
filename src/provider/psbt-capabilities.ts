...
export class ProviderSigningCapabilityError extends Error {
  constructor(
    message: string,
    public readonly code: ProviderSigningCapabilityErrorCode,
  ) {
    super(message)
...
export function parseProviderPsbtSigningCapabilities(value: unknown): ProviderPsbtSigningCapabilities | null {
  ...
  const maxRequests = psbtBatch && typeof psbtBatch === 'object' && !Array.isArray(psbtBatch)
