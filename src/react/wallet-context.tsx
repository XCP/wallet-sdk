...
async function checkProof(proof: ConnectionProof, addr: string): Promise<ProofStatus> {
  ...
  const check = await validateProof(proof, window.location.origin, addr, {
    verifySignature: async (message, signature, address) => {
      try { return verifyDeclaredConnectionSignature(proof, message, signature, address) } catch { return false }
    },
  })
...
export interface WalletContextValue {
  ...
  signPsbt: {
    ...
    (hex: string, signInputs?: Record<string, number[]>, sighashTypes?: number[], inscription?: SignPsbtParams['inscription']): Promise<string>
  }
...
export function WalletProvider({
   ...
   */
  const refreshAddressMetadata = useCallback(async (
    active: string,
    knownAddresses?: Awaited<ReturnType<XcpWallet['getAddresses']>>,
  ) => {
    const addresses = knownAddresses === undefined
      ? await walletRef.current?.getAddresses() ?? null
      : knownAddresses
  ...
  const connect = async () => {
    ...
    try {
      ...
      if ((e as { code?: number })?.code === 4001) eventsRef.current?.onRejected?.()
    } finally {
