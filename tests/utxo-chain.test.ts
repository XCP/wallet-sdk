...
describe("cross-tab UTXO journal", () => {
  it("turns safe change into a complete inputs_set entry", () => {
    ...
    expect(pendingChangeInputs(ADDRESS)).toEqual([`${BROADCAST_TXID}:0:1000:${ADDRESS_SCRIPT}`]);
  });
  ...
  it("sees a later write immediately and removes change once it is spent", () => {
    ...
    expect(recentlySpentUtxos(ADDRESS)).toEqual([`${BROADCAST_TXID}:1`, `${PREVIOUS_TXID}:0`]);
    expect(pendingChangeInputs(ADDRESS)).toEqual([`${NEXT_TXID}:2:1500:${ADDRESS_SCRIPT}`]);
  });
...
describe("address transaction lock", () => {
  it("uses one case-normalized lock name for a bech32 address", () => {
    expect(addressTransactionLockName("BC1QEXAMPLE")).toBe(addressTransactionLockName("bc1qexample"));
  });
